...
Stream.iterate(analyze("sin x"), p -> p.derivate()).map(e -> e.eval("x", 0)).limit(10).collect(ndclc()) // sin 导数
Stream.iterate(analyze("cos x"), p -> p.derivate()).map(e -> e.eval("x", 0)).limit(10).collect(ndclc()) // cos 导数
