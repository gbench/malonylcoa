...
public class SrchUtils {
	...
	public static IRecord parse2Term(final String termline) {
		...
		Function<String, Tuple2<String, BooleanClause.Occur>> name_parser = (nameline) -> {
			...
			switch (grp1 == null ? "" : grp1) {// 操作符选择
			case "+":
				op = BooleanClause.Occur.MUST;
				break;
			case "-":
				op = BooleanClause.Occur.MUST_NOT;
				break;
			case "#":
				op = BooleanClause.Occur.FILTER;
				break;
			default:
				op = BooleanClause.Occur.SHOULD;
			}// switch
	...
	public static Tuple2<BooleanClause.Occur, Query> parse2Query(Tuple2<String, Term> p) {
		...
		Query qry = new TermQuery(p._2);// 查询选项
		switch (item.str("type")) {// term 类型
		case "=":
			qry = new TermQuery(p._2);
			break; //
		case "*":
			qry = new WildcardQuery(p._2);
			break;
		case "-":
			qry = new PrefixQuery(p._2);
			break;
		case "~":
			qry = new FuzzyQuery(p._2);
			break;
		default:
			qry = new TermQuery(p._2);
			break;
	...
	public static Collector<Tuple2<String, Object>, List<Tuple2<String, Term>>, BooleanQuery> bool_query_clc = bool_query_clc(
			(ll, builder) -> ll.stream().map(p -> parse2Query(p)).forEach(e -> builder.add(e._2, e._1)));

	/**
	 * 把一个文本描述解析为一个 数据定义
	 * 
	 * @param line field 的文本描述
	 */
	public static Field parse2Field(final String line) {
		// 仿照sql 的数据定义方式
		// field type
		final var matcher = Pattern.compile("^\\s*([^\\s]+)\\s+([^\\s]+)\\s+([^\\s]+)\\s+([^\\s]+)\\s*$").matcher(line);
		if (matcher.matches()) {
//            final String field = matcher.group(1);
//            final String docValuesType = matcher.group(2);
//            final String store = matcher.group(3);
//            final String index = matcher.group(4);
	...
	public static Document rec2doc(final IRecord rec) {
		...
		rec.tupleS().filter(p -> p._2 != null).map(p -> { // 依次遍历各个字段定义:kvpair(p)
			...
			switch (field_type_and_name.charAt(0)) {
			case '$':
				return nosave // 直接return 因此就不需要 break了
