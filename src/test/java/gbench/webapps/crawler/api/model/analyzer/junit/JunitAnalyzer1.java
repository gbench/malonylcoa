...
public class JunitAnalyzer1 {
    ...
    public YuhuanAnalyzer getYunHuan(final String corpusDir) {
        ...
        @SuppressWarnings("resource") final var yuhuan = new YuhuanAnalyzer().addProcessor(new ILexProcessor() { // 创建一个 分词器
    ...
    public void foo() {
        ...
        final var fileHome = FT("$0/java/gbench/webapps/crawler/api/model/analyzer/junit/data/docs", prefix);
        final var corpusDir = FT("$0/java/gbench/webapps/crawler/api/model/analyzer/junit/data/corpus", prefix);
        Stream.of(new File(fileHome).listFiles()).forEach(file -> {
