...
public class YuhuanAnalyzer extends Analyzer {
	...
	protected TokenStreamComponents createComponents(String fieldName) {
		...
		final var tokenizer = new Tokenizer() {// 分的词是存放在tokenizer的属性里面的
			...
			private LexemeAttribute lexemeAttr = addAttribute(LexemeAttribute.class);
	 ...
	 * @return 分词明细 symbol,category,meaning,含有tags 集合标签
	 */
	public <U> Stream<U> analyzeS(final String line, Function<TokenProfile, U> mapper) {
		if (yuhuanLexer == null) {
			yuhuanLexer = new StreamLexer();
		}
		List<TokenProfile> words = yuhuanLexer.analyze(line);
	 ...
	 * @param line      字段名 fieldName 组件对于使用同一个分词器进行分词，即 重用策略ReuseStrategy 具有标记意义。
	 */
	 ...
	 * 这就是袋袋裤的设计思想的程序体现。<br>
	 */
	private IRecord attributes = REC();// 分词器的属性状态，
}