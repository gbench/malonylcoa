...
public class Trie<T> {
    ...
    public Trie<T> loop_getTrie(final List<T> points) {
        ...
        while(pitr.hasNext()) {
           if (!tval.equals(pval)) return null;
    ...
    public static<T> Stream<T> getPath(final Trie<T> trie){
        if(trie==null || trie.getParent()==null)return Stream.empty();
        else return Stream.concat(getPath(trie.getParent()), Stream.of(trie.getValue()));
    }
    ...
    public Stream<T> getPath(){
        if(this.getParent()==null)return Stream.empty();
        else return Stream.concat(getPath(this.getParent()), Stream.of(this.getValue()));
    }
    ...
    public static <T> Trie<T> create(final List<List<T>> lines) {
        ...
        for (var line : lines) addPoints2Trie(trie,line);
        return trie;
    }
    ...
    public static <T> Trie<T> addPoints2Trie(final Trie<T> trie, final Stream<List<T>> lines) {
        ...
        lines.forEach(line->{// 依次加入数据点集行
            var cur_trie = trie;
            for (var piont : line) cur_trie = cur_trie.addPoint(piont);
            ar.set(cur_trie);
        });
