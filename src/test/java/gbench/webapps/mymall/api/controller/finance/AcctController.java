...
public class AcctController {
	 ...
	 * 数据演示
	 * <p>
	 * http://localhost:6010/finance/acct/trial_balance?company_ids=1&keys=ledger_id,acctnum,warehouse,product,drcr
	 * 
	 * @return IRecord
	 */
	...
	public Mono<IRecord> trial_balance(final @Param Integer[] company_ids, final @Param String keys) {
		...
		final var _keys = Optional.ofNullable(keys).orElse("ledger_id,acctnum,warehouse,product,drcr").split(",");
		final var root = fa.trialBalance(_keys);
		fa.dump(root);
		final var json = root.json( //
				(sb, node) -> gbench.util.lisp.IRecord.FT("{\"name\":\"$0 --> $1\", \"value\":\"$1\", \"children\":[", //
						node.getName(), node.attrval()), //
				(sb, node) -> "]}");

		return Mono.just(REC("code", "0", "data", json));
	}
