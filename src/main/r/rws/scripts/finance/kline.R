...
# options("sqlquery.rb.instrument"="rb2601");ohlc(endtime=.1200, keys=4:8) |> chartSeries(theme="white"); addSMA(c(1, 3, 5, 10, 10, 20, 30, 60))
# 默认合约&添加移动均线的绘图(从options中清除合约代码)
# options("sqlquery.rb.instrument"=NULL);ohlc(endtime=.1200, keys=4:8) |> chartSeries(theme="white"); addSMA(c(1, 3, 5, 10, 10, 20, 30, 60))
