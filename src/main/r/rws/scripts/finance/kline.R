...
kchart <- \(instrument=getOption("sqlquery.rb.instrument", "rb2701"), startime=0, endtime=24, periods=c(10, 20, 30), tsp=NULL) {
  kct <- rbx.tse(instrument, startime, endtime, tsp=tsp) |> evalq() |> sqldframe(x=OHLCV.1M) |> df2xts(3:8) |> chartSeries(name=instrument) # K线图表
  ...
  ( \(n=periods, col=seq(n)+1) mapply(\(n, col, lgd) newTA(SMA, Cl, on=1, legend=lgd, col=col) (n=.(n)) |> bquote() |> eval(), n, col, lgd=paste0("MA", n)) ) () # 多周期移动平均，legend 的名称参数被替换
  kct |> invisible()
}
