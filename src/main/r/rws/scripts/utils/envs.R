  ...
  setNames(rep_len(., length(keys)), keys) |> (\(rec) { # 采用循环填充rep_len的方式构造记录结构
      ...
      Reduce(\(acc, k, v=acc[[k]]) { # 对记录值进行私人定制
        if(identical("##tbl", k)) { # 表名字段处理
          .v <- gsub("\\s*", "", v) |> (\(.) ifelse(is.null(.) || is.na(.) || grepl("^$", .) || length(.) < 1, getOption("sqlquery.rb.instrument", "rb2605"), .)) () # 默认合约表
          ...
          if(tryCatch(hms(.v, quiet=T)) |> is.na()) { # 时间非法值
            acc[[k]] <- switch(k, "#startime"=ftm(Sys.time()-getOption("sqlquery.rb.interval", 3)*3600), "#endtime"=ftm(Sys.time()), .v) # 默认时间处理
          } # if
        } # if 时间字段
        acc # 返回累计值
