 ...
 * 理解认识，也就是 文化观念上 对这 形式/结构 的约束和限制，即: XX形式 代表了 YY意义）, 换句话说 <br>
 * 意义 的 常见解释 就是 把 结构 -&gt; 实现出 特定目的 的 功能用途，这里的 ‘-&gt;’ 读作 映射到 . <br>
 * 所以 意义 是一种 在 什么情况下 使用 某种（些）结构 的 习惯约定。 它由设计 决定。 <br>
 * 就像 房子 可以 使用 砖木结构 也可以 钢筋结构 两种不同的 结构 却常常用来完成 同样的 功能 <br>
 * 对应相同的意义。同样 的 钢筋结构 不仅可以 建房子 还可以 键大船, 也就是 同样的 结构 可以用于 实现不同的 功能 <br>
