...
public class PinyinUtil {
	...
	public static String path(String file,Class<?>clazz) {
		...
		Class<?> cls = clazz == null?PinyinUtil.class:clazz;
		try {
			path = cls.getClassLoader()
	...
	public static String [] getPinyin(String s) {
		...
		s= convertChineseToPinyin(s);// 转换成拼音文字
		String t = p.matcher(s).replaceAll("_$1");//添加下划线进行分割
	 ...
	 * @return
	 */
	public static String getPinyinShort(String s) {
		return Arrays.stream(getPinyin(s)).filter(e->e.length()>0)
			.map(e->e.charAt(0)+"")
	 ...
	 * @return
	 */
	public static String getPinyinLong(String s) {
	 ...
	 */
	public static List<String> getPinyins(String s) {
		List<String> ss = new ArrayList<>();
		if(s==null)return ss;// 不会返回null
		String pp[] = getPinyin(s);
		if(pp==null||pp.length<1)return ss;
		StringBuffer s1= new StringBuffer();//缩略
		StringBuffer s2= new StringBuffer();//全拼
